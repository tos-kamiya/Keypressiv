...
SLIDE_OPTIONS = ("trans-none", "trans-horiz", "trans-vert", "trans-vert-stack")
...
_trans_tbl = {
    (1, "trans-none"): ("", False),
    (-1, "trans-none"): ("", False),
    (1, "trans-horiz"): ("right", False),
    (-1, "trans-horiz"): ("left", False),
    (1, "trans-vert"): ("bottom", False),
    (-1, "trans-vert"): ("top", False),
    (1, "trans-vert-stack"): ("bottom", True),
    (-1, "trans-vert-stack"): ("top", True),
}

def parse_trans(delta, attrs):
    if delta > 0: delta = 1
    elif delta < 0: delta = -1
    for attr in attrs:
        if attr.startswith("trans-"):
            a_s = _trans_tbl.get((delta, attr), None)
            if a_s is not None:
                return a_s
            else:
                console.log("invalid page-transition specifier: " + attr + "\n");
    if delta > 0:
        return "right", False
    else:
        return "left", False
